...
const { getLogs } = require('./utils/logs'); // New
...
const memberSearchRoutes = require("./routes/membersearch96");
app.use(memberSearchRoutes);
...
let connectedIPs = new Map(); // IP -> timestamp
let totalRequests = 0;
let startTime = new Date();
let requestsInMinute = [];
let recentLogs = []; // For errors

// Socket.io
io.on('connection', (socket) => {
  const clientIP = socket.handshake.address || 'unknown';
  connectedIPs.set(clientIP, new Date().toISOString());
  io.emit('ipUpdate', { ip: clientIP, action: 'add' });
  console.log(`Client connected from ${clientIP}`);

  socket.on('disconnect', () => {
    connectedIPs.delete(clientIP);
    io.emit('ipUpdate', { ip: clientIP, action: 'remove' });
...
// Broadcast every 5s
setInterval(() => {
  const metrics = getMetrics(totalRequests, requestsInMinute, startTime, os.cpus());
  metrics.connectedIPs = Array.from(connectedIPs.entries()).map(([ip, ts]) => ({ ip, ts: new Date(ts).toLocaleTimeString() }));
  metrics.logs = getLogs(recentLogs); // Broadcast logs
  io.emit('metricsUpdate', metrics);

  // Simulate log on high load
  if (metrics.cpuLoad > 50 || metrics.heapUsedPercent > 60) {
    recentLogs.push({ type: 'warn', message: `High load detected at ${new Date().toLocaleTimeString()}`, timestamp: Date.now() });
  }

  // Clean up
  const now = Date.now();
  requestsInMinute = requestsInMinute.filter(time => now - time < 60000);
  recentLogs = recentLogs.slice(-10); // Keep last 10
}, 5000);
...
app.get('/dashboard', (req, res) => {
  const metrics = getMetrics(totalRequests, requestsInMinute, startTime, os.cpus());
  metrics.connectedIPs = Array.from(connectedIPs.entries()).map(([ip, ts]) => ({ ip, ts: new Date(ts).toLocaleTimeString() }));
  metrics.logs = getLogs(recentLogs);
...
app.get('/api/load', (req, res) => {
  totalRequests++;
  requestsInMinute.push(Date.now());
  const ip = req.ip;
  if (!connectedIPs.has(ip)) {
    connectedIPs.set(ip, new Date().toISOString());
    io.emit('ipUpdate', { ip, action: 'add' });
  }
